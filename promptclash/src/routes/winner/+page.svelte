  ...
  onMount(async () => {
    ...
    try {
      ...
      if (result.tie) {
        ...
        const res = await fetchWinningResponse(gameId);
        if (res) {
          winningResponse = res.winningResponse;
          losingResponse = res.losingResponse;
        }
      }
    } catch (err) {
...
{:else}
  ...
  {#if winningResponse && losingResponse}
    <div>
      <p>
        <strong>{winningResponse.username}</strong> wins this round, with response:
      </p>
