<script lang="ts">
  ...
  import {
    fetchCurrentGameId,
    calculateGameScores,
    fetchWinningResponse,
    fetchAllPlayersScores,
    calculatePromptScores,
  } from "$lib/api";

  let gameId: number;
  let losingResponse: any = null;
  let winningResponse: any = null;
  let tiePlayers = [];
  ...
  let finalMode = false;
  let finalScores: any[] = [];
  let result: any = null; // result from calculatePromptScores for the current prompt
  ...
  function nextPrompt() {
    ...
    // If we still have more prompts => go to next voting
    if (newIndex < allPrompts.length) {
      goto(`/voting?gameId=${gameId}&promptIndex=${newIndex}`);
    } else {
      // no more prompts => final scoreboard
      goto(`/winner?gameId=${gameId}&final=true`);
    }
  }
</script>
